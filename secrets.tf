...
## ExternalSecret - Syncs GitHub token from AWS Secrets Manager
## Deployed in Stage 3 after SecretStore is created (controlled by deploy_kubernetes_manifests variable)
## Note: Creates secret named konvu-git-credentials (required by konvu-controller)
resource "kubernetes_manifest" "github_token_secret" {
  count = var.deploy_kubernetes_manifests ? 1 : 0
  ...
  manifest = yamldecode(<<-YAML
    ...
    spec:
      ...
      data:
        - secretKey: token
          remoteRef:
            key: ${var.github_token_secret_name}
  YAML
  )
